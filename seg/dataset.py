...
class LiverDataset(data.Dataset):
    def __init__(self, state, transform=None, target_transform=None):
        self.state = state
        self.train_root = r"/media/wit513/wjmecho/awesome-semantic-segmentation-pytorch-master/datasets/voc/VOC2012/train"
        self.val_root = r"/media/wit513/wjmecho/awesome-semantic-segmentation-pytorch-master/datasets/voc/VOC2012/test"
    ...
    def getDataPath(self):
        ...
        pics = []
        masks = []
        n = len(os.listdir(root))  # 因为数据集中一套训练数据包含有训练图和mask图，所以要除2
        for i in range(n):
            img = os.path.join(root, "%d.tif" % i)  # liver is %03d
...
class DriveEyeDataset(data.Dataset):
    def __init__(self, state, transform=None, target_transform=None):
        self.state = state
        self.aug = True
        self.root = r'/media/wit513/wjmecho/awesome-semantic-segmentation-pytorch-master/datasets/voc/VOC2012'
        self.pics, self.masks = self.getDataPath()
        self.img_paths = None
        self.mask_paths = None
        self.train_img_paths, self.val_img_paths,self.test_img_paths = None,None,None
    ...
    def getDataPath(self):
        self.train_img_paths = glob(self.root + '/JPEGImages/*')
        self.train_mask_paths = glob(self.root + '/SegmentationClass/*')
        # print(self.train_img_paths)
        # print(self.train_mask_paths)
        self.val_img_paths = glob(self.root + '/valimg/*')
        self.val_mask_paths = glob(self.root + '/vallab/*')
        self.test_img_paths = glob(self.root + '/testimg/*')
        self.test_mask_paths = glob(self.root + '/testimg/*')
        assert self.state == 'train' or self.state == 'val' or self.state == 'test'
        if self.state == 'train':
            return self.train_img_paths, self.train_mask_paths
        if self.state == 'val':
